...
def choose(Willie, trigger):
    ...
    Willie.debug(__file__, log.format(u"=============="), u"verbose")
    Willie.debug(__file__, log.format(u"Module called."), u"verbose")
    # Parse the provided arguments into a list of strings
    Willie.debug(__file__, log.format("Trigger args: ", trigger.args), u"verbose")
    __, __, list = trigger.args[1].partition(u' ')
    # Test for csv or space separated values
    if u',' in trigger.args[1]:
        Willie.debug(__file__, log.format('list: ', list), u"verbose")
        args = list.split(u',')
        Willie.debug(__file__, log.format('args: ', args), u"verbose")
    else:
        args = list.split()
    # Strip the strings
